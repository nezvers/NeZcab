...
def  main():
  ...
  installer = "\installer\NeZcab Installer.exe"
   
  if demo:
    installer = "\installer\NeZcab Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\NeZcab manual.pdf" 
  ]
  ...
  ZIPNAME = "NeZcab-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "NeZcab-v" + FULLVERSIONSTR + "-win-demo.zip"
