...
if %1 == 1 (echo Making NeZcab Windows DEMO VERSION distribution ...) else (echo Making NeZcab Windows FULL VERSION distribution ...)
...
REM - msbuild NeZcab-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild NeZcab.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild NeZcab.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\NeZcab.aaxplugin\Contents\Win32\NeZcab.aaxplugin --out .\build-win\aax\bin\NeZcab.aaxplugin\Contents\Win32\NeZcab.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\NeZcab.aaxplugin\Contents\x64\NeZcab.aaxplugin --out .\build-win\aax\bin\NeZcab.aaxplugin\Contents\x64\NeZcab.aaxplugin
...
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q /cc ".\installer\NeZcab.iss"
goto END-is

:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q /cc ".\installer\NeZcab.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "NeZcab Installer" ".\installer\NeZcab Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\NeZcab Installer.exe" ".\installer\NeZcab Demo Installer.exe"
REM -del ".\installer\NeZcab Installer.exe"
REM -)
